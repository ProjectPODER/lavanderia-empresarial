...
const diccionario = [
    ...
    'aliment', '\\balpha\\b', 'alquil', 'alternativ', 'alumbrado', 'aluminio', 'ambient', 'american', 'anal(i|í)?y?tic', 'an(a|á)li?y?sis', 'anillos', 'an(o|ó)nim',
    'aplica(c|t)', '\\bapps?\\b', 'applied', '\\b(a|á)rea\\b', 'arquitect', 'arrenda', '\\barte?\\b', 'art(i|í)culo', 'asegura', 'asesor', '\\baxtel\\b',
    'aserradero', 'as(f|ph)alt', 'ass?ocia', 'atenci(o|ó)n', 'atlantic', 'av(i|í)cola', 'ayuntamient', '\\baudio', 'authority', '\\bauto', 'auxiliar',
    '\\bbanc(a|o)\\', 'banquete', 'bebida', 'benefici?encia', 'biblioteca', 'bienes', '\\bbio\\b', 'bmw', '\\bboards?\\b', 'bombas', 'bombero', 'british',
    'bufete', 'business', '\\bbioware\\b',
    ...
    'cliente', '\\bclima(s|te)?\\b', 'cl(i|í)nic(a|o)', 'club', 'coahuila', 'colecciones', 'coll?ectiv', 'colegio', 'combusti', 'comm?erci(al|o)', 'comisariado',
    ...
    'conserva(c|t)ion', 'consorcio?', '(c|k)ons?truc', 'constit', 'consult', 'consumibles', 'contabilidad', 'contempor(a|á)ne', 'continental', 'contrac?t',
    'control', 'cooperativ', 'coordina', '\\bcourier\\b', 'corpora(c|t)(ion|ión|iv|e)', '\\bcorpation\\b', 'creaciones', 'cultur', 'compressor',
    'convocatoria', '\\bcivil\\b',
    ...
    'evolution\\b', '\\bendowment\\b', '\\beditl\\b', '\\bespacios?\\b',
    'f(a|á)brica', 'factory', 'family', 'farmac', 'federal', 'fedex', 'ferreter', 'festival', 'fianza', 'fideicomiso', 'fiesta', 'filtro', '\\bfinal\\b', 'financ',
    'films', 'filmoteca', 'formaci(o|ó)n', 'formas', 'f(o|ó)rmul', 'foto', 'fo?unda(c|t)i(o|ó)n?', 'franquicia', 'frenos', 'frutas', '\\bfuerza', 'fumiga(c|s)',
    'funci(o|ó)n', '\\bfilm\\b', 'funeral', '\\bfraude\\b', '\\bfashion\\b',
    'ganader', 'gallery', 'geotecnia', 'general', 'gen(e|é)tic', 'geren(cia|te)', 'global', 'gobierno', 'gourmet', 'gr(a|á)(f|ph)ic', 'graf(i|í)a\\b',
    'growth', 'gruas', '\\bgrupo', 'guarder(i|í)a', '\\bgasolin',
    ...
    'im(a|á)gen?', '(im)?permea(b|v)i?l', 'implement', 'import', 'imprenta', 'impres', 'impuls', 'incorp', 'independ', '\\bindex\\b', '\\bind(i|í)gena\\b', '\\bindust',
    'inform(a|á)ti(c|o|v)', 'infra', 'ingenier', 'ingredient', 'iniciativa', 'inmobiliari', 'inn?ova', 'inspec', 'instal', 'institu', 'instrument',
    'insumo', 'insurance', 'intell?igen', 'integra', '\\binter(a|b|c|f|k|m|n|s|v)', 'inventario', 'investment', 'investiga', 'investor', 'inyecc',
    ...
    'limpieza', 'ling(u|ü)(i|í)stic', 'llantas', 'log(i|í)stic', '\\blogo', 'lubri', '\\bllantera\\b', '\\blaser\\b',
    ...
    '\\bpro\\b', 'pron(o|ó)stic', 'property', 'protec', 'proveedor', 'pro(y|j)ecc?t', 'proxy', 'publi(c|sh)?', 'purific', 'peninsula', '\\bpartners?\\b',
    'playground', '\\bphi\\b', '\\bplg\\b',
    'quantum', 'qu(i|í)mic',
    ...
    'sinergia', 's(i|y)stem', '\\bsmurfit\\b', 'socie(dad|ty|te)', 'software', 'solu(c|t)i(o|ó)n', 'soporte', '\\bspa\\b', 'spanish', '\\bsport', 'squash', '\\bstate\\b',
    'sub(a|á)rea', 'sucursal', 'suministro', 'supervis', 'supply', 'suprem(a|o)', 'surtidor', 'steakhouse', '\\be?studio\\b', 'synergy', '\\bsurvey\\b', '\\bsigns\\b',
    '\\btabasco\\b', 'taller', 'tapete', 'taquiza', 'teatro', '\\btech\\b', 't(e|é)ch?n(o|i)', 'techos', 'tejidos', 'telecom', 'tel(e|é)fon', 'televis',
    'tensi(o|ó)n', 'terracer(i|í)a', 'terrestre', '\\bthe\\b', 'theat(er|re)', 'ticket', '\\btienda\\b', 'tijuana', 'tokyo', 'torno', 'tortas', 'tortill', 'toyota',
    'traducci(o|ó)n', 'traffic', 'transform', 'transport', 'tratamiento', 'travel', 'to?ur(i|í)s', 'training', 'tr(o|ó)nic', '\\btrust(ee)?\\b', 'turbo',
    'turbine', '\\btv\\b', '\\btrabajador', '\\btrabajo\\b', '\\btoner\\b',
...
module.exports = function(string, debug=false){
  ...
  // H.G.
  if( string.match(/(,|\.|\s)H\.?\s?G(\.|,){0,2}(\s\(.*\))?$/i) ) { return true }
  // I.A.P.
  if( string.match(/(,|\.|\s)I\.?\s?A\.?\s?P(\.|,){0,2}(\s\(.*\))?$/i) ) { return true }
  // L.L.C.
  if( string.match(/(,|\.|\s)L\.?\s?L\.?\s?C(\.|,){0,2}(\s\(.*\))?$/i) ) { return true }
  // L.P.
  ...
  // S.A.
  if( string.match(/(,|\.|\s)S:?\.?\s?A\s?(\.|,){0,2}(\s\(.*\))?(\sde)?$/i) ) { return true }
  // S.A.B., S.A.S.
  if( string.match(/(,|\.|\s)S\.?\s?A\.?\s?(B|S)(\.|,){0,2}(\s\(.*\))?$/i) ) { return true }
  // S.A.S.C.I.
  if( string.match(/(,|\.|\s)S\.?\s?A\.?\s?S\.?\s?C\.?\s?I(\.|,){0,2}(\s\(.*\))?(\sde)?$/i) ) { return true }
  // S.C.
  if( string.match(/(,|\.|\s)S,?\.?\s?C\s?(\.|,){0,2}(\s\(.*\))?$/i) ) { return true }
  // S.C.A., S.C.P., S.C.L.
  if( string.match(/(,|\.|\s)S\.?\s?C\.?\s?(L|P|A)(\.|,){0,2}(\s\(.*\))?$/i) ) { return true }
  // S.D.R.L.
  if( string.match(/(,|\.|\s)S\.?\s?D\.?\s?R\.?\s?L(\.|,){0,2}(\s\(.*\))?$/i) ) { return true }
  // S.L.
  ...
  // Z.O.O., D.O.O.
  if( string.match(/(,|\.|\s)(Z|D)\.?\s?O\.?\s?O(\.|,){0,2}(\s\(.*\))?$/i) ) { return true }

  // * de cv
  if( string.match(/,?\s?d\s?e\.?\s?\.?c,?\.?\s?c?:?-?\.?\s?v,?:?-?(\.|,){0,2}(\s\(.*\))?$/i) ) { return true }
  // sa de cv, ss de cv, sa de sv, sa de dv
  if( string.match(/,?\s?S\s?,?\.?\s?(A|S)\s?,?\.?\s?D?\s?E?\.?,?\s?\.?(C|S|D)?X?\s?,?\.?\s?V\s?,?\.?\s?V?\s?,?(\.|,){0,2}(\s\(.*\))?$/i) ) { return true }
  // sa de cc/dc
  ...
  // s de pr de ri
  if( string.match(/,?\sS?\s?D\s?E\sP\.?\s?R\.?\s?D\s?E\sR\.?\s?I\.?\s?(\.|,){0,2}(\s\(.*\))?$/i) ) { return true }
  // s de rl
  ...
  // Inicia con DIST. (distibuidora o similar)
  if( string.match( /^dist\.?\s/i ) ) { return true }
  // Inicia con EJ. (ejido)
  if( string.match( /^ej\.?\s/i ) ) { return true }
  // Inicia con ESP. (especialistas o similar)
  if( string.match( /^esp\.?\s/i ) ) { return true }
  // Inicia con GOB. (gobierno o similar)
  if( string.match( /^gob\.?\s/i ) ) { return true }
  // Inicia con INST. (instituto, institución, etc)
  if( string.match( /^inst\.?\s/i ) ) { return true }
