...
module.exports = function(string, debug=false){
  ...
  // A.C.
  if( string.match(/(,|\s)a\.?\s?c\.?$/i) ) { return true }
  if( string.match(/,?\sasociaci[oó]n\s?civil$/i) ) { return true }
  // A.G.
  if( string.match(/(,|\s)a\.?\s?g\.?$/i) ) { return true }
  // B.V.
  if( string.match(/(,|\s)b\.?\s?v\.?$/i) ) { return true }
  // GMBH GNBH
  if( string.match(/(,|\s)g\.?\s?(m|n)\.?\s?b\.?\s?h\.?$/i) ) { return true }
  // I.A.P.
  if( string.match(/(,|\s)i\.?\s?a\.?\s?p\.?$/i) ) { return true }
  // S.C.
  if( string.match(/(,|\s)s\.?\s?c\.?$/i) ) { return true }
  // S.A.
  if( string.match(/(,|\s)s\.?\s?a\.?$/i) ) { return true }
  // S.A.B.
  if( string.match(/(,|\s)s\.?\s?a\.?\s?b\.?$/i) ) { return true }
  // S.C.P.
  if( string.match(/(,|\s)s\.?\s?c\.?\s?p\.?$/i) ) { return true }
  // S.P.A.
  if( string.match(/(,|\s)s\.?\s?p\.?\s?a\.?$/i) ) { return true }
  // S.R.L.
  if( string.match(/(,|\s)s\.?\s?r\.?\s?l\.?$/i) ) { return true }
  // * de cv
  if( string.match(/,?\s?d\s?e\.?\s?\.?c,?\.?\s?c?\.?\s?v,?\.{0,2}$/i) ) { return true }
  // sa de cv, ss de cv
  if( string.match(/,?\s?s\s?,?\.?\s?(a|s),?\.?\s?d?\s?e?\s?\.?c?,?\.?\s?v,?\.{0,2}$/i) ) { return true }
  // sa de c, sa de v
  if( string.match(/,?\s?s\s?,?\.?\s?a,?\.?\s?d?\s?e?\s?\.?(c|v),?\.{0,2}$/i) ) { return true }
  // S. De G.C. De I.P.
  ...
  // spr de ri/rl
  if( string.match(/,?\ss\.?\s?p\.?\s?r\.?\s?d\s?e\sr\.?\s?l?\.?\s?m?\.?\s?i?\.?$/i) ) { return true }
