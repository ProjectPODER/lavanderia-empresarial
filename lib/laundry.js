...
module.exports = function(string) {
  return normalize(string)
    ...
    // remove phrase (AMPLIACION AL CONTRATO) and others from string before anything else
    .replace(/\(AMPLIACION\s.*\)/, '')
    // trim non-alphanumeric (including whitespace) unless `.` at end (account for unicode)
    .replace(/^[^a-zA-Z0-9\u00C0-\u00FF]+|[^.a-zA-Z0-9\u00C0-\u00FF]+$/g, '')
    // replace underscores with spaces
    .replace(/_/g, ' ')
    // replace unwanted characters that might break IDs and URLs
    .replace(/[¿?"()'´`=°º¨“”}{’\]\[<>–~\|…¡!]/g, '')
    ...
    // FIXME we would have to blacklist known siglge letter words like `y`
    ...
    // sa de cv
    .replace(/,?\s?s\s?\.?\s?a\.?\s?d?\s?e?\s?\.?c?\.?\s?v\.{0,2}$/i, ', S.A. de C.V.')
    // S. De G.C. De I.P.
